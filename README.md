...
11. **Optimal Scale for Consumer CPUs**: Models in the 1B to 3B parameter range (e.g., Llama 3.2 1B/3B) represent the "Pareto optimal" point for HADES. Specifically, **Llama 3.2 (1B)** provides 100% precision on technical nuances (pluralization/units) while maintaining the 2.5s latency profile of sub-billion models.
12. **The "Thinking" Overhead**: Native-thought models (like Qwen 3.5 0.8B) are currently impractical for local HADES caches on CPU, as their internal reasoning chains increase latency by >70x compared to traditional dense architectures.
