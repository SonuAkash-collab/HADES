...
11. **Optimal Scale for Consumer CPUs**: Models in the 1B to 3B parameter range (e.g., Llama 3.2 1B/3B) represent the "Pareto optimal" point for HADES. Specifically, **Llama 3.2 (3B)** provides 95% precision, while **Qwen 3 (0.6B)** remains the best sub-1B champion due to its balanced synthesis.
12. **Model Flavor over Parameter Count**: Our audit found that **Instruct-tuned** models (Qwen 3) significantly outperform **Coder-tuned** models (Qwen 2.5 Coder) for HADES synthesis. Coder models tend to hallucinate tool-calls (e.g., phantom JSON) instead of providing direct answers, while Llama 3.2 (1B) exhibits "alignment cowardice," returning INSUFFICIENT DATA despite 100% retrieval hits.
