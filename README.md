...
11. **Optimal Scale for Consumer CPUs**: Models in the 3B parameter range (e.g., Llama 3.2) represent the "Pareto optimal" point for HADES on CPU-only hardware. They offer the reasoning precision required for 95% accuracy without the synthesis stalls or memory pressure failures seen in larger models (4B+).
12. **Prompt Processing vs. Generation Bottlenecks**: On CPU inference, the primary bottleneck for standard RAG is the "Time to First Token" caused by processing thousands of context tokens. HADES eliminates this by serving <20 tokens of context, shifting the computational load from the slow LLM inference stage to the highly efficient, cacheable graph retrieval stage.
