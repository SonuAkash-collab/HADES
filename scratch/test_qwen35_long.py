...
def test_model(model_name):
    ...
    print(f"--- Testing {model_name} (Increased Token Limit) ---")
    for q in questions:
        print(f"[{q['label']}] Query: {q['query']}")
        try:
            # We increase num_predict to 1024 to allow for "Thinking" overhead
            response = ollama.chat(
                model=model_name,
                messages=[{'role': 'user', 'content': f"Answer in one short sentence. Do not think, just provide the answer.\nContext: {q['context']}\nQuestion: {q['query']}"}],
                options={'temperature': 0.0, 'num_predict': 1024}
            )
            content = response['message']['content'].strip()
            # If the model still outputs <thought> blocks, we'll see them here
